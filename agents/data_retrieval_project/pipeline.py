...
from agents.data_retrieval_project.query_builder import ProjectQueryBuilder, IntentExtractor


class ProjectDomainAgent:
    def __init__(self, client):
        self.domain_key = "project"
        self.display_name = "Project Agent"
        ...
        self.query_builder = ProjectQueryBuilder(client=client, model="gpt-5.1")
    ...
    def _db_executor(self, sql: str) -> list[dict]:
        """Bridge for ProjectQueryBuilder to run SQL via ExecutionEngine."""
    ...
    def _format_algorithm_output(self, intent: dict, sql: str | None, db_result: dict) -> str:
        ...
        return "\n".join(
            [
                ...
                "",
                "Algorithm followed:",
                "1. Read the user query against the available project schema.",
    ...
    def execute_events(self, question: str):
        ...
        if intent.get("intent") == "unknown":
            yield self._event(
                "error",
                f"{self.display_name} could not understand the query. Please rephrase with a clearer location, metric, or date range.",
            )
            return {
                "status": "unknown",
                "domain": self.domain_key,
        ...
        if route == "clarify" or intent.get("needs_clarification"):
            questions = intent.get("clarification_questions") or [
                "Please rephrase your question with the key location, metric, or date range."
            ]
