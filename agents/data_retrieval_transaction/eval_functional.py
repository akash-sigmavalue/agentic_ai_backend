"""
Functional Evaluation Framework for ReAct SQL Agent  —  v1.0
=============================================================
...
─────
  Set environment variables before running:
    ...
    LANGSMITH_PROJECT=react-sql-eval                   (optional)
...
try:
    ...
    TRANSACTION_QUERY_SCHEMA = "mock_schema"
...
TEST_CASES: list[dict] = [
    ...
    # ── TC_001  Basic single-location summary ─────────────────────────────────
    {
        "id":          "TC_001",
        "category":    "basic",
        "query":       "total sales in Baner",
        # --- Intent expectations ---
        "expected_intent": {
            ...
            "metrics": [{"alias": "total_sales_value"}],
        },
        # --- SQL pattern expectations ---
        # ALL of these regex patterns must match in the generated SQL
        "expected_sql_patterns": [
            r"SUM\s*\(\s*agreement_price\s*\)",
            r"ILIKE\s+'%Baner%'",
        ],
        # NONE of these must appear in the generated SQL
        "forbidden_sql_patterns": [
            r"=\s*'Baner'",          # must use ILIKE, not =
            r"\bDROP\b",
            r"\bDELETE\b",
        ],
        # --- Result expectations ---
        "min_row_count":           1,
        "max_iterations_expected": 2,
    },

    # ── TC_002  Multi-entity comparison — entity completeness critical ─────────
    {
        "id":          "TC_002",
        "category":    "entity_completeness",
        "query":       "compare Baner and Hinjewadi total sales units sold rate per sqft",
        "expected_intent": {
        ...
        "expected_sql_patterns": [
            # Both locations must be in SQL
            r"ILIKE\s+'%Baner%'",
            r"ILIKE\s+'%Hinjewadi%'",
            # All 3 metrics
            r"SUM\s*\(\s*agreement_price\s*\)",
            r"COUNT\s*\(\s*\*\s*\)",
            r"10\.764",                         # rate formula
            r"NULLIF",                           # division safety
        ],
        "forbidden_sql_patterns": [
            r"=\s*'Baner'",
            r"=\s*'Hinjewadi'",
    ...
    # ── TC_003  Three-location comparison ─────────────────────────────────────
    {
        "id":          "TC_003",
        "category":    "entity_completeness",
        "query":       "compare Baner, Wakad, and Kothrud by units sold",
        "expected_intent": {
            "analysis_type": "comparison",
            "entities": {
                "locations": [
    ...
    # ── TC_004  Trend query — must have time dimension in GROUP BY ─────────────
    {
        "id":          "TC_004",
        "category":    "trend",
        "query":       "show quarterly sales trend in Wakad",
        "expected_intent": {
            "analysis_type": "trend",
            "time_series":   True,
            "entities": {
        ...
        "expected_sql_patterns": [
            r"GROUP\s+BY",
            # Must group by a time dimension
            r"\b(year|quarter|EXTRACT|DATE_TRUNC)\b",
            r"ORDER\s+BY",
            r"ILIKE\s+'%Wakad%'",
        ],
        "forbidden_sql_patterns": [],
        "min_row_count":           2,       # trend must have multiple rows
        "max_iterations_expected": 3,
    },

    # ── TC_005  Ranking query ─────────────────────────────────────────────────
    {
        "id":          "TC_005",
        "category":    "ranking",
        "query":       "top 5 localities by total sales value in Pune",
        "expected_intent": {
    ...
    # ── TC_006  Rate per sqft formula check ───────────────────────────────────
    {
        "id":          "TC_006",
        "category":    "metric_formula",
        "query":       "what is the average rate per sqft in Hinjewadi",
        "expected_intent": {
            "analysis_type": "summary",
            "entities": {
                "locations": [{"value": "Hinjewadi", "semantic_level": "locality"}]
            },
            "metrics": [{"alias": "rate_per_sq_ft"}],
        },
        "expected_sql_patterns": [
            r"net_carpet_area_sq_m",
            r"10\.764",
            r"NULLIF",
            r"ROUND",
        ],
    ...
    # ── TC_007  Data quality filters must be present ──────────────────────────
    {
        "id":          "TC_007",
        "category":    "data_quality",
        "query":       "total transactions in Baner last year",
        "expected_intent": {
            "analysis_type": "summary",
            "entities": {
                "locations": [{"value": "Baner", "semantic_level": "locality"}]
        ...
        "expected_sql_patterns": [
            r"agreement_price\s*>=\s*1",    # data quality filter
            r"ILIKE\s+'%Baner%'",
        ],
        "forbidden_sql_patterns": [],
        "min_row_count":           0,        # might be 0 rows if no data for year
        "max_iterations_expected": 2,
    },

    # ── TC_008  ILIKE enforced — never equality for text ──────────────────────
    {
        "id":          "TC_008",
        "category":    "ilike_enforcement",
        "query":       "sales in Kharadi",
        "expected_intent": {
            "analysis_type": "summary",
            "entities": {
        ...
        "forbidden_sql_patterns": [
            r"=\s*'[Kk]haradi'",     # exact equality is forbidden
        ],
        "min_row_count":           0,
        "max_iterations_expected": 2,
    },

    # ── TC_009  Multi-metric — all must appear ────────────────────────────────
    {
        "id":          "TC_009",
        "category":    "multi_metric",
        "query":       "give me total sales value, units sold, and rate per sqft for Aundh",
        "expected_intent": {
            "analysis_type": "summary",
            "entities": {
    ...
    # ── TC_010  Distribution query ────────────────────────────────────────────
    {
        "id":          "TC_010",
        "category":    "distribution",
        "query":       "breakdown of sales by property type in Pune",
        "expected_intent": {
...
class IntentEvaluator:
    ...
    def evaluate(self, test_case: dict) -> IntentEvalResult:
        ...
        # We compare location values case-insensitively.
        exp_locs  = {
            e["value"].lower()
            for e in (exp.get("entities") or {}).get("locations", [])
        }
        pred_locs = {
            e["value"].lower()
            for e in (predicted.get("entities") or {}).get("locations", [])
        }

        if exp_locs:
            tp                = len(exp_locs & pred_locs)
            entity_recall     = tp / len(exp_locs)
            entity_precision  = tp / max(len(pred_locs), 1)
            f1 = (
                2 * entity_recall * entity_precision
                / max(entity_recall + entity_precision, 1e-9)
            )
            missing = exp_locs - pred_locs
            extra   = pred_locs - exp_locs
            if missing:
...
class SQLEvaluator:
    ...
    def evaluate(
        ...
        exp_locs = [
            e["value"]
            for e in (intent.get("entities") or {}).get("locations", [])
        ]
        entity_hits: list[bool] = []
        for loc in exp_locs:
            # Location can appear in ILIKE '%Baner%' or similar
            found = bool(
...
class EndToEndEvaluator:
    """
    Runs the full agent (IntentExtractor + TransactionQueryBuilder ReAct loop)
...
class LangSmithEvaluator:
    ...
    def __init__(
        ...
        db_executor:    Callable[[str], list[dict]],
        model:          str   = "gpt-4.1",
        dataset_name:   str   = "react-sql-agent-eval",
        project_prefix: str   = "react-v3",
    ) -> None:
    ...
    def _get_or_create_dataset(self) -> Any:
        ...
        dataset = self.ls_client.create_dataset(
            ...
            description="ReAct SQL agent functional evaluation dataset",
        )
    ...
    def _make_evaluators(self) -> list[Callable]:
        ...
        def eval_entity_completeness(run, example):
            ...
            expected_locs  = [
                e["value"]
                for e in (example.outputs.get("expected_intent") or {})
                          .get("entities", {})
                          .get("locations", [])
            ]
            missing = [
            ...
            score   = 1.0 if not missing else len(expected_locs - set(missing)) / max(len(expected_locs), 1)
            return {
...
class EvalRunner:
    ...
    def _mock_executor(sql: str) -> list[dict]:
        ...
        """
        return [
            {"location_name": "Baner",     "total_sales_value": 5_00_00_000, "transaction_count": 120},
            {"location_name": "Hinjewadi", "total_sales_value": 3_50_00_000, "transaction_count": 88},
        ]
    ...
    def run(self, test_cases: list[dict] | None = None) -> EvalReport:
        ...
        print(f"\n{'='*60}")
        print(f" ReAct SQL Agent — Functional Evaluation  ({self.mode.upper()} mode)")
...
CONFIG = {
    ...
    "langsmith_project":  os.getenv("LANGSMITH_PROJECT", "react-sql-eval"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Functional evaluation for ReAct SQL agent"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--cases",
        nargs="+",
        help="Run only specific test case IDs, e.g. --cases TC_001 TC_002",
    )
    parser.add_argument(
        "--save",
        default=CONFIG["report_json"],
        help="Path to save JSON report (default: eval_report.json)",
    )
