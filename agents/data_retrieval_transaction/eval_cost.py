"""
Cost Evaluation Framework for ReAct SQL Agent  —  v1.0
=======================================================

Measures the second evaluation layer for the transaction ReAct agent:
...
--------
  ...
  python backend/agent/transaction/eval_cost.py --mode mock --cases TC_001 TC_002

  # Full run against DATABASE_URL
  python backend/agent/transaction/eval_cost.py --mode full

  # Compare models on the same cases
  python backend/agent/transaction/eval_cost.py --mode mock --models gpt-4.1-mini gpt-4.1

  # Provide current pricing explicitly, per 1M tokens
  python backend/agent/transaction/eval_cost.py --input-price 0.40 --output-price 1.60
"""
...
def build_mock_executor() -> Callable[[str], list[dict]]:
    ...
    def executor(sql: str) -> list[dict]:
        return [
            {
                "location_name": "Baner",
                "total_sales_value": 50_000_000,
                "units_sold": 120,
                "rate_per_sq_ft": 12_500,
                "transaction_count": 120,
            },
            {
                "location_name": "Hinjewadi",
                "total_sales_value": 35_000_000,
                "units_sold": 88,
                "rate_per_sq_ft": 10_800,
...
if HAS_LANGSMITH:
    @traceable(
        name="transaction_cost_eval_case",
        run_type="chain",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cost evaluation for transaction ReAct SQL agent"
    )
    ...
    parser.add_argument("--save", default="cost_eval_report.json")
    ...
    parser.add_argument(
        "--langsmith-project",
        default=os.getenv("LANGSMITH_PROJECT", "react-sql-cost-eval"),
